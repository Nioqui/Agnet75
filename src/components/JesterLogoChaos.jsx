 ...
 * JesterLogoChaos.jsx
 *
 * Special version of the Jester Logo for the Chaos phase.
 * Differences from original:
 * - eyeState is permanently "CHAOS"
 * - Mouse tracking is disabled
 * - Blink frequency is increased
 * - Pupil movement is wilder and less predictable
 * - Outer scale jitter added via GSAP on the SVG root
 */

export default function JesterLogoChaos(props) {
  const pupilRef = useRef(null);
  const svgRef = useRef(null);
  const instanceId = useRef(Math.random().toString(36).substr(2, 9));

  const maskId = `eye-blink-mask-chaos-${instanceId.current}`;
  const coverId = `blink-cover-chaos-${instanceId.current}`;
  ...
  useEffect(() => {
    ...
    let ctx = gsap.context(() => {
      ...
      const performChaosAction = () => {
        ...
        if (actionRand < 0.7) {
          // SUDDEN JUMP
          const jumpLimitX = LIMITS.x * 1.5;
          const jumpLimitY = LIMITS.y * 1.5;
          ...
          chaosTl.to(pupilRef.current, {
            ...
            onComplete: () => {
              gsap.to(pupilRef.current, {
                x: clamp(
                  pupilRef.current._gsap ? pupilRef.current._gsap.x : 0,
                  LIMITS.x,
                ),
                y: clamp(
                  pupilRef.current._gsap ? pupilRef.current._gsap.y : 0,
                  LIMITS.y,
                ),
                duration: 0.04,
              });
          ...
          // MICRO JITTER
          const jitterCount = 5 + Math.floor(Math.random() * 8);
          ...
          // STARING PHASE
          chaosTl.to({}, { duration: 0.2 + Math.random() * 0.3 });
        }

        const nextChaosDelay = 30 + Math.random() * 120;
        chaosTimer = setTimeout(performChaosAction, nextChaosDelay);
      };

      performChaosAction();

      // --- OUTER SCALE JITTER (Specific to Chaos version) ---
      ...
      function scheduleNextBlink() {
        ...
        let delay = 0.3 + Math.random() * 1.5;
        blinkTimer = gsap.delayedCall(delay, performBlink);
      }
